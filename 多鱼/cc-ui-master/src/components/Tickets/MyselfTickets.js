...
class MyselfTickets extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if(nextProps.pageNo===1){
      this.listData = nextProps.securityList;
      this.len = nextProps.total;
    ...
    }else if (nextProps.securityList !== this.props.securityList) {
  ...
  useTicket(id) {
   ...
   //赠送卡券
   transfer(id){
      return () => {this.props.history.push({ pathname: '/ticketsTransfer/'+id })};
   }
   ...
   use1(id){
    return () => {
      alert('', '确认使用该券？', [
        ...
        { text: '确认', onPress: () => this.useTicket(id) },
      ])
    }
   }
  ...
  render() {
            ...
            <div style={{width:'100vw'}}>
              <div className="rFirst">
                <div className="rF-des">
                  ...
                  {isUsed==="Y"||inTimeFlag==="N"?"":<div className="rF-opr" onTouchEnd={this.transfer(rowData.id)}><span>赠送</span></div>}
    ...
    return (
      <div style={{ paddingTop: '100px',background:'#F6F6F6' }}>
        <div className='headStyle' style={{ height: '100px'}}>
          ...
          <p style={{ fontSize: '1.167rem', display: 'inline-block' }}>我的卡券</p>
        </div>
        <ListView
          ref={el => this.lv = el}
...
export default connect(mapStateToProps)(MyselfTickets);
